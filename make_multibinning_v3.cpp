...
void make_multibinning_v3(int run_N=000000, TString Target="C", int Hadron_pid=211, TString type="data"){
    ...
    if      (run_location=="own_pc"){output_location="output/"+torus_pol+"/"+subdir+"/"+Target+"/"+run_N_str+thrown_dir;}
    ...
    if      (run_location=="own_pc"){input_location="output/"+torus_pol+"/"+subdir+"/"+Target+"/"+run_N_str+thrown_dir;}
    else if (run_location=="farm"){input_location="/volatile/clas12/antorad/rge/MR_analysis/"+type+"/pass1/"+torus_pol+"/dc/"+Target+"/"+run_N_str;}
